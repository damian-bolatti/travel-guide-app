...
describe('PlaceRepositoryImpl', () => {
  ...
  const mockPlaceDTOs = [
    { key: 'barcelona', place: { type: 'monument', name: 'Sagrada Familia', coordinates: [41.4036, 2.1744] } },
    { key: 'paris', place: { type: 'monument', name: 'Eiffel Tower', coordinates: [48.8584, 2.2945] } },
  ];

  const mockMappedPlaces: Place[] = [
    { type: 'monument', name: 'Sagrada Familia', coordinates: [41.4036, 2.1744] },
  ];
  ...
  it('should fetch and return mapped places filtered by city key', async () => {
    ...
    expect(mockedFetchGraphQL).toHaveBeenCalledWith(expect.stringContaining('query'));
  ...
  it('should propagate errors from fetchGraphQL', async () => {
    ...
    await expect(repository.getByCityKey('barcelona')).rejects.toThrow('GraphQL Error');
  });
});
