...
describe('CityRepositoryImpl', () => {
  ...
  const mockCities: City[] = [
    { id: 1, key: 'paris', name: 'Paris', nativeName: 'Paris', currency: 'EUR', language: 'fr' },
    { id: 2, key: 'barcelona', name: 'Barcelona', nativeName: 'Barcelona', currency: 'EUR', language: 'es' },
  ];

  beforeEach(() => {
    jest.clearAllMocks();
    mockedFetchGraphQL.mockResolvedValue({ allCities: mockCityDTOs });
    mockedMapCityFromDTO
      .mockImplementation((dto: any) => mockCities.find((c) => c.key === dto.key));
  });
